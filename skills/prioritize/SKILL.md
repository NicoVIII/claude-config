...
- Security alerts: `gh api repos/<repo>/dependabot/alerts?state=open --jq '[.[] | {severity: .security_advisory.severity, package: .dependency.package.name, summary: .security_advisory.summary, created_at}]'` — severity drives ranking (one critical outweighs many lows), so pull it now rather than a bare count. Where this 403s, the error body's `message` field is authoritative ("Dependabot alerts are disabled for this repository" means exactly that) — `gh` also prints a boilerplate hint to refresh auth scopes; ignore it, treat the repo as alerts-disabled, and move on.
