...
- **Pick the tree.** A workflow that only makes sense in the current repo goes in that repo's `.claude/skills`, where it ships with the repo; ask when unsure. Everything below holds there too, with the project repo standing in wherever a step names `~/.claude`: its own commit, its own checks, and its `.claude/README.md` table only if it already keeps one — the log rates the skill either way.
...
- Add the maturity-table row, starting at 🚧 WIP; if the skill pairs with existing ones, extend the Workflows section. For a project skill both go where that repo documents its skills — the row only if a `.claude/README.md` table already exists.
