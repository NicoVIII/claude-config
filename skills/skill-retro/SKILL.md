...
One retro covers one skill: never fold another skill's friction into this one. Shared machinery is the exception — a doc, script, or log the run touched or relied on. Fix a defect there in this pass whether the run created it or merely exposed it, and say why: routing it elsewhere means some other skill's next run rediscovers it, and a note in a commit message is where such a finding dies.
