...
Also report accretion: run `dotnet run --project ~/.claude/bin/runlog -- ratio <skill>` and pass on its line. Don't act on it — removing text in the same pass that fixes friction is how the friction always wins, which is why `/skill-compact` is separate.
