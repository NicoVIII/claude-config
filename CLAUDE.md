...
- When I answer a question with a question, the direction is unsettled: answer mine and stop there — don't decide and implement in the same turn.
...
- A branch with exactly one commit merges with plain `git merge` instead — it fast-forwards, so no merge commit restates the single commit below it, and falls back to a merge commit if the default branch moved meanwhile. Never `--ff-only`: that forces a rebase, putting the commit on a base its check run never saw. That commit's own subject carries the issue reference, since it may be all there is. `git log --first-parent` still reads as one line per unit of work; undo a unit with `git revert -m 1 <merge>` or plain `git revert <sha>`, whichever form it took.
